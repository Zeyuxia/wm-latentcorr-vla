...
def get_trainer_callbacks(lightning_config, config, logdir, ckptdir, logger):
    ...
    # Normalize ImageLogger save_dir to be under current workdir (logdir)
    try:
        if "batch_logger" in callbacks_cfg and "params" in callbacks_cfg.batch_logger:
            save_dir = callbacks_cfg.batch_logger.params.get("save_dir", None)
            if save_dir is None:
                callbacks_cfg.batch_logger.params["save_dir"] = logdir
            else:
                # If a relative path is provided, prefix with workdir (logdir)
                if not os.path.isabs(str(save_dir)):
                    callbacks_cfg.batch_logger.params["save_dir"] = os.path.join(logdir, str(save_dir))
    except Exception:
        pass
