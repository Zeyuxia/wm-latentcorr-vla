...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument("--resume", "-r", type=str, default=None, help="path to checkpoint for resuming training")
...
if __name__ == "__main__":
    ...
    # Set CUDA device early to prevent cross-GPU context allocation
    ...
    if args.train:
        try:
            ...
            if "strategy" in lightning_config and lightning_config['strategy'].startswith('deepspeed'):
                ...
                # breakpoint()
                logger.info("<Training in the Mode set the config>") ## original default is ddp sharded
                trainer.fit(model, data, ckpt_path=resume_ckpt)
        except Exception:
            #melk()
            raise