...
RUN_TAG="stage1_unified_failure_multitask_resume900_dp_gpu0123_${TIMESTAMP}"
...
SESSION="stage1_unified_resume900_dp_gpu0123"

mkdir -p "${OUTPUT_ROOT}" "${OUTPUT_DIR}" "${LOG_ROOT}" "${LOG_DIR}"

tmux has-session -t "${SESSION}" 2>/dev/null && tmux kill-session -t "${SESSION}"
tmux new-session -d -s "${SESSION}" "cd /data/zhenyangfan/RoboTwin && CUDA_VISIBLE_DEVICES=0,1,2,3 PYTHONUNBUFFERED=1 /data/zhenyangfan/RoboTwin/policy/ACT_LatentCorr/.venv-act-latent/bin/python -m policy.ACT_LatentCorr.train_stage1_unified_failure_multitask_latent \
  ...
  --resume_ckpt '/data/zhenyangfan/RoboTwin/policy/ACT_LatentCorr/outputs/formal_runs/stage1_unified_failure_multitask/stage1_unified_failure_multitask_resume650_rebalfg_rank0fail_gpu0123_20260513_024859/stage1_unified_epoch_0900.pt' \
