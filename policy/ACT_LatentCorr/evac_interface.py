...
class EvacLatentTeacher:
    """
    Minimal EVAC latent interface for stage-1 warmup.

    This wrapper only exposes final VAE latent encoding from real frames.
    DiT rollout latent for closed-loop stage will be added in step-2.
    """

    def __init__(self, evac_ckpt: str, evac_config: str, device: str | torch.device = "cuda:0"):
        self.device = torch.device(device)
        print(f"[evac] init start device={self.device}", flush=True)
        self.model, self.cfg = self._load_model(evac_ckpt, evac_config)
        print("[evac] model loaded", flush=True)
        self.model.eval()
        for p in self.model.parameters():
            p.requires_grad = False
        print("[evac] init complete", flush=True)

    def _load_model(self, evac_ckpt: str, evac_config: str) -> tuple[Any, Any]:
        if not os.path.exists(evac_ckpt) and "/EVAC_cache/" in evac_ckpt:
            evac_ckpt_alt = evac_ckpt.replace("/EVAC_cache/", "/EVAC/")
            if os.path.exists(evac_ckpt_alt):
                evac_ckpt = evac_ckpt_alt
        if not os.path.exists(evac_config) and "/EVAC_cache/" in evac_config:
            evac_config_alt = evac_config.replace("/EVAC_cache/", "/EVAC/")
            if os.path.exists(evac_config_alt):
                evac_config = evac_config_alt

        config = OmegaConf.load(evac_config)
        config.model.pretrained_checkpoint = evac_ckpt
        print("[evac] config loaded", flush=True)

        # EVAC mixes `evac.*` imports with flat `utils.*` / `lvdm.*` imports.
        # Add both repository root and package root to sys.path so all variants resolve.
        evac_repo_override = os.environ.get("EVAC_REPO_ROOT", "").strip()
        if evac_repo_override:
            evac_pkg_root = os.path.realpath(evac_repo_override)
        else:
        ...
        print("[evac] general_utils imported", flush=True)

        model = instantiate_from_config(config.model)
        print("[evac] instantiate_from_config complete", flush=True)
        model = load_checkpoints(model, config.model, ignore_mismatched_sizes=False)
        print("[evac] checkpoints loaded", flush=True)
        model = model.to(self.device)
        print("[evac] model moved to device", flush=True)
        return model, config

    @torch.no_grad()
    def encode_image(self, image: torch.Tensor) -> torch.Tensor:
        """
        Encode RGB image to EVAC final VAE latent.

        Args:
            image: (B, 3, H, W) in [0, 1].
        Returns:
            z: (B, C, H', W') final VAE latent.
        """
        if image.ndim != 4:
            raise ValueError(f"image must be (B,3,H,W), got {image.shape}")
        x = image.to(device=self.device, dtype=torch.float32)
        sample_h, sample_w = tuple(self.cfg.data.params.train.params.sample_size)
        x = F.interpolate(x, size=(sample_h, sample_w), mode="bilinear", align_corners=False)
        x = (x * 2.0) - 1.0
        z = self.model.encode_first_stage(x, mode=True)
        if z.ndim != 4:
            raise RuntimeError(f"Expected 4D latent, got shape {tuple(z.shape)}")
        return z.float()
    ...
    def _prepare_rollout_inputs(
        ...
        # EVAC expects pose rollout starting from current state.
        fk_poses = []
        grippers = []
        ...
        for a in act_raw:
            fq = fk.forward(a[:6], a[7:13])
            fk_poses.append((fq["left"][0], fq["left"][1], fq["right"][0], fq["right"][1]))
            grippers.append((float(a[6]), float(a[13])))
        ...
        for i, ((lp, lq, rp, rq), (lg, rg)) in enumerate(zip(fk_poses, grippers)):
            ...
            lq_xyzw = np.array([lq[1], lq[2], lq[3], lq[0]], dtype=np.float32)
            rq_xyzw = np.array([rq[1], rq[2], rq[3], rq[0]], dtype=np.float32)
            if lq_xyzw[3] < 0:
                lq_xyzw = -lq_xyzw
            if rq_xyzw[3] < 0:
                rq_xyzw = -rq_xyzw
            ...
            gripper_arr[i] = [float(lg) * 120.0, float(rg) * 120.0]
        ...
        # EVAC expects camera poses as (view, time, 4, 4). We only use one view here.
        ...
        video_list = memories.unsqueeze(1)  # (3, 1, n_prev, h, w)
        ...
        # Minimal single-chunk rollout mirrors the first chunk in ACWMLatentDiffusion.inference().
        video = torch.cat(
            (
    ...
    def rollout_latent_from_actions_batch(
        ...
        """
        Batched EVAC latent rollout for stage-2.

        Args:
            curr_image: (B, 3, H, W) or (3, H, W) BGR tensor in [0, 1].
            curr_qpos_raw: (B, 14) or (14,) raw qpos.
            action_prefix_raw: (B, K, 14) or (K, 14) raw action prefix.
            raw_data: per-sample raw episode metadata dict or list of dicts.
        Returns:
            (B, C, H', W') latent of the last valid rollout step for each sample.
        """
        ...
        # Match EVAC inference defaults for single-chunk rollout.
