...
CKPT=${CKPT_DIR}/stage1_unified_epoch_0500.pt
...
MASTER_TAG=${MASTER_TAG:-actmt500_gpu01234567_pred_eval100_novideo_$(date +"%Y%m%d_%H%M%S")}
...
for idx in "${!TASKS[@]}"; do
  if [ "$idx" -lt "$START_TASK_INDEX" ]; then
    continue
  fi
  task="${TASKS[$idx]}"
  ...
  if LATENT_CKPT_PATH="$CKPT" \
    ...
    EVAC_CKPT="$EVAC_CKPT" \
    RUN_TAG="$run_tag" \
    /data/zhenyangfan/RoboTwin/policy/ACT_LatentCorr/launch_parallel_eval.sh; then
    ...
    echo "[$(date '+%F %T')] launch failed for task=$task, continuing with next task"
  fi
done

echo "[$(date '+%F %T')] all epoch500 multitask eval finished"
