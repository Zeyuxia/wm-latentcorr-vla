...
CKPT_DIR=/data/zhenyangfan/RoboTwin/policy/ACT_LatentCorr/outputs/formal_runs/stage1_unified_failure_multitask/stage1_unified_failure_multitask_fulltable_lctok05_resume50_wandb_gpu0123_20260510_224203
CKPT=${CKPT_DIR}/stage1_unified_epoch_0250.pt
...
MASTER_TAG=${MASTER_TAG:-actmt250_gpu4567_pred_eval100_novideo_remaining_$(date +"%Y%m%d_%H%M%S")}
...
for task in "${TASKS[@]}"; do
  ...
  while true; do
    active=$( (tmux ls 2>/dev/null || true) | rg -F "${run_tag}_" | wc -l | tr -d ' ' )
    if [ "$active" = "0" ]; then
      break
    fi
    sleep 20
...
echo "[$(date '+%F %T')] all epoch250 remaining multitask eval finished"
