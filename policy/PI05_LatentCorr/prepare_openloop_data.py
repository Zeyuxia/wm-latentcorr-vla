...
def _selected_cameras(camera_mode: str) -> list[tuple[str, str]]:
...
def convert_raw_episode_to_pi0_format(
    ...
    *,
    ...
    episode_dir = output_root / f"episode_{episode_id}"
    ...
    qpos = []
    actions = []
    selected_cameras = _selected_cameras(camera_mode)
    camera_buffers = {camera_name: [] for camera_name, _ in selected_cameras}
    left_arm_dims = []
    right_arm_dims = []
    ...
    for step in range(left_gripper_all.shape[0]):
        ...
        if step != left_gripper_all.shape[0] - 1:
            qpos.append(state)
            for camera_name, robotwin_key in selected_cameras:
    ...
    with h5py.File(hdf5_path, "w") as f:
        ...
        for camera_name, _ in selected_cameras:
...
def create_lerobot_dataset(
    ...
    *,
    ...
    camera_names = [camera_name for camera_name, _ in _selected_cameras(camera_mode)]
    features = {
        "observation.state": {
            "dtype": "float32",
            "shape": (len(motors), ),
    ...
    hdf5_files = sorted(processed_dir.glob("episode_*/episode_*.hdf5"))
...
def main() -> None:
    ...
    processed_dir = Path(
        ...
        or (
            DEFAULT_PROCESSED_DATA_ROOT
            / f"{args.task_name}-{args.task_config}-{args.expert_data_num}-{args.camera_mode}"
        )
    ).expanduser().resolve()
    ...
    print(f"[PI0_LatentCorr] raw_root={raw_root}")
    print(f"[PI0_LatentCorr] processed_dir={processed_dir}")
    for episode_id in range(args.expert_data_num):
        ...
        print(f"[PI0_LatentCorr] converted episode_{episode_id}")
    ...
    print(f"[PI0_LatentCorr] finished LeRobot export for repo_id={args.repo_id}")
