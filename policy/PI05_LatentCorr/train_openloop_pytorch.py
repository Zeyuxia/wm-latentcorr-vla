...
try:
    ...
    from policy.PI05_LatentCorr.common import (
        DEFAULT_ASSETS_BASE_DIR,
        DEFAULT_CAMERA_MODE,
        DEFAULT_CHECKPOINT_BASE_DIR,
        build_train_config,
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--project-name", default="RoboTwin_PI05_LatentCorr")
...
def main() -> None:
    ...
    logging.info("[PI05_LatentCorr] starting PyTorch open-loop training")
    logging.info("[PI05_LatentCorr] rank=%s world=%s device=%s", get_rank(), os.environ.get("WORLD_SIZE", "1"), device)
    logging.info("[PI05_LatentCorr] train_config=%s", args.train_config_name)
    logging.info("[PI05_LatentCorr] repo_id=%s", args.repo_id)
    logging.info("[PI05_LatentCorr] exp_name=%s", args.exp_name)
    logging.info("[PI05_LatentCorr] pytorch_weight_path=%s", Path(args.pytorch_weight_path).resolve())
