...
OUTPUT_DIR=${OUTPUT_DIR:-${SCRIPT_DIR}/outputs/stage1/${TRAIN_CONFIG_NAME}/${TASK_NAME}_${TASK_CONFIG}_${EXPERT_DATA_NUM}_${CAMERA_MODE}}
...
REFERENCE_GLOBAL_BATCH_SIZE=${REFERENCE_GLOBAL_BATCH_SIZE:-${BATCH_SIZE}}
...
if [ -z "${EVAC_CKPT}" ] || [ -z "${EVAC_CONFIG}" ]; then
  echo "[PI05_LatentCorr] EVAC_CKPT and EVAC_CONFIG are required for Stage 1"
  exit 1
fi
...
"${PYTHON_BIN}" -m policy.PI05_LatentCorr.train_stage1 \
  ...
  ${SAMPLES_PER_EPOCH:+--samples-per-epoch "${SAMPLES_PER_EPOCH}"} \
  "${EXTRA_ARGS[@]}"
