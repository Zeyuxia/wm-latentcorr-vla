...
class LiberoEnv(gym.Env):
    ...
    def __init__(
        ...
        task_id: int,
        task_suite_name: str,
        camera_name: str | Sequence[str] = "agentview_image",
        obs_type: str = "pixels",
        ...
        # Map raw camera names to "image".
        # The preprocessing step `preprocess_observation` will then prefix these with `.images.*`,
        # following the LeRobot convention (e.g., `observation.images.image`).
    ...
    def set_batch_index(self, batch_index: int, num_envs: int):
        """
        Set which init_state to use based on current batch and env position.
        
        Called externally before each batch's reset() to ensure each episode
        uses a different init_state.
        
        Formula: init_state_id = batch_index * num_envs + episode_index
        
        Example with batch_size=5, n_episodes=50:
          - Batch 0: envs use init_states [0,1,2,3,4]
          - Batch 1: envs use init_states [5,6,7,8,9]
          - ...
          - Batch 9: envs use init_states [45,46,47,48,49]
        """
        if self._init_states is not None:
            self._init_state_id = (batch_index * num_envs + self.episode_index) % len(self._init_states)
            # Debug print
            print(f"[LiberoEnv] task_id={self.task_id}, episode_index={self.episode_index}, "
                  f"batch={batch_index}, init_state_id={self._init_state_id}")
