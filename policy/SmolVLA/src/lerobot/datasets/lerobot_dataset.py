...
class LeRobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.randomize_episode_instructions = os.getenv(
            "LEROBOT_RANDOMIZE_TASK_FROM_EPISODE_INSTRUCTIONS", ""
        ).lower() in {"1", "true", "yes", "on"}
        self.episode_instructions = self._load_episode_instructions()
    ...
    def _load_episode_instructions(self) -> dict[int, list[str]]:
        metadata_path = self.root / "meta" / "episode_instructions.json"
        ...
        for item in payload:
            ...
            instructions = [str(instruction) for instruction in item.get("instructions", []) if str(instruction)]
            if instructions:
    ...
    def __getitem__(self, idx) -> dict:
        ...
        if self.randomize_episode_instructions and ep_idx in self.episode_instructions:
            item["task"] = random.choice(self.episode_instructions[ep_idx])
        return item
