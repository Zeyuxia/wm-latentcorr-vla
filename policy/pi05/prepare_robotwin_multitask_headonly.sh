...
REPO_ID=${REPO_ID:-robotwin/multitask5_${TASK_CONFIG}_${EXPERT_DATA_NUM}_headonly}
PROCESSED_DIR=${PROCESSED_DIR:-${SCRIPT_DIR}/outputs/processed_data/multitask5-${TASK_CONFIG}-${EXPERT_DATA_NUM}-${CAMERA_MODE}}
...
if [ "${CAMERA_MODE}" != "head_only" ]; then
  echo "[PI05_RobotWin] this helper is intended for single-view/head_only PI0.5 base training" >&2
  exit 1
fi
...
echo "[PI05_RobotWin] multitask base data plan"
echo "  tasks: ${TASK_ARR[*]}"
...
cfg = build_train_config(
    ...
    exp_name="prepare_robotwin_multitask_headonly",
...
if [ "${PREPARE_PER_TASK_ASSETS}" = "true" ]; then
  for task in "${TASK_ARR[@]}"; do
    task_repo_id="robotwin/${task}_${TASK_CONFIG}_${EXPERT_DATA_NUM}_headonly"
    task_processed_dir="${SCRIPT_DIR}/outputs/processed_data/${task}-${TASK_CONFIG}-${EXPERT_DATA_NUM}-${CAMERA_MODE}"
...
cfg = build_train_config(
    train_config_name=os.environ["TRAIN_CONFIG_NAME"],
    repo_id=os.environ["REPO_ID"],
    exp_name="prepare_robotwin_per_task_headonly",
    camera_mode=os.environ["CAMERA_MODE"],
    asset_id=os.environ["REPO_ID"],
    assets_base_dir=os.environ["ASSETS_BASE_DIR"],
)
...
echo "[PI05_RobotWin] finished base multitask data and norm stats"
